new Vue({
    ...
    computed: {
        lengthThumbPosition: function () {
            return (((this.settings.length - 6) / (this.settings.maxLength - 6))
                * 100);
...
strength: function () {
    ...
    else if ((count.upperCase && count.numbers) || (count.upperCase &&
        count.symbols) || (count.numbers && count.symbols)) {
        weight.combo = 15;
    }
...
methods: {
    ...
    generatePassword() {
        var lettersSetArray = ["a", "b", "c", "d", "e", "f", "g", "h", "i",
        ...
        lowecase
        // first, fill the password array with letters, uppercase and
        ...
        for (i = 0; i < this.settings.digits; i++) {
            ...
            /* remove position from digitsPositionArray so we make sure to
            the have the exact number of digits in our password
